...
exports.register=async(req,res)=>{
    try{
        ...
        if(!msg){
            ...
            return res.send({message: 'The username is currently in use, choose another'});
            }
        }else{
    ...
    try{
        ...
        if(!msg){
            ...
            if(userExist&& await checkPassword(params.password, userExist.password)){
                ...
                return res.send({message: 'Username of password incorrect'});
            }
        }else{
            return res.status(400).send(msg);
        }
...
exports.update=async(req, res)=>{
    try{
        ...
        if(permission===false)return res.status(401).send({message: 'Unauthorized to update this user, you can only update your count'});
        else{
...
exports.delete=async(req,res)=>{
    try{
        ...
        const permission=await checkPermission(userId, req.user.sub);
        if(permission===false){
            return res.status(401).send({message: 'Unauthorized to delete this user, you can only delete your count'});
        }else{
...
exports.saveUser=async(req,res)=>{
    try{
        ...
        if(data.role=='CLIENT'||data.role=='ADMIN'){
            ...
            return res.send({message: 'Type a suitable role "ADMIN" or "CLIENT"'});
