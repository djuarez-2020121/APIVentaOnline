...
const ShoppingCart=require('../models/carShopping.model');
...
exports.searchUser=async(username)=>{
    try{
        const exist=User.findOne({username: username}).lean()
        return exist;
...
exports.findShoppingCart=async(shoppingCartUser)=>{
    try{
        const exist=ShoppingCart.findOne({ user: shoppingCartUser }).lean();
        return exist;
    }catch (err){
        console.log(err);
        return err;
